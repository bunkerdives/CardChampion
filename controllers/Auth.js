...
var Auth = {
    ...
    , register : function( req, res ) {
        ...
        // TODO before registering the account, make sure no one has that username (check Passport + Profiles)
        ...
        Account.register( new Account({ username : username }), req.body.password, function(err, account) {
            ...
            if( ! Auth.userExists( username, profileModel ) ) {
                ...
                // at this point, we created a passport login yet we cannot create a new profile account
                res.send('Error');
            }
            
            // TODO I used to set req.session.username here... may need to be necessary again
            res.send( username );
    ...
    , newUser : function( data, profileModel ) {
        ...
        userInstance.save( function(err) {
            // TODO handle user save error case gracefully
        } );
        
    }
    
    // userExists - return true if there's a profile with the specified username already, false for viceversa
    , userExists : function( username, profileModel ) {
        
        profileModel.find( { email : username }, function(err, data) {
            if( err ) {
                return false;
            } else {
                if( data == [] ) {
    ...
    // getAuthority - check if the request connection has been authenticated.
    //                If so, return the username, else, return undefined
    , getAuthority : function( req ) {
